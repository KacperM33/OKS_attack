...
def run_experiment(exp_id, experiments_folder, pose_est_cfg, pose_est_ckpt):
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        log_print(f"Experiment ORACLE_BBOX_DET", log_file)
    ...
    oks_attack(name=name, folder_exp_name=folder_exp_name, detector_cfg=detector_cfg, detector_ckpt=detector_ckpt, 
             pose_est_cfg=pose_est_cfg, pose_est_ckpt=pose_est_ckpt, input_path=input_path, mat_kp_path_gt=mat_kp_path_gt, 
             trial=trial, experiments_folder=experiments_folder, log_path=log_path)
...
def oks_attack(name, folder_exp_name, detector_cfg, detector_ckpt, pose_est_cfg,
             pose_est_ckpt, input_path, mat_kp_path_gt, trial, experiments_folder, log_path):
    with open(log_path, "a", encoding="utf-8") as log_file:

        # script path
        script = "../attacks/OKS_attack_det.py"
